...
def country_iso_code_xml(country_iso_code: str) -> str:
    ...
    result = template.render({'country_iso_code': country_iso_code})
    # print(result)
    return result

def country_name_xml(country_name: str) -> str:
    template = env.get_template('/templates/xml/country_name.xml')
    result = template.render({'country_name': country_name})
    # print(result)
    return result

def xsd_response(operation: str) -> XMLSchema11:
    envelope_xsd = env.get_template('/templates/xsd/envelope.xsd')
    country_iso_code_xsd_response = envelope_xsd.render({'operation_xsd': f'{operation}.xsd', 'operation': operation})
    # country_iso_code_xsd_response = envelope_xsd.render(
    #     {'operation_xsd_file': 'CountryNameResponse.xsd', 'operation': 'CountryNameResponse'})

    with open("./templates/xsd/temp.xsd", "w") as f:
        f.write(country_iso_code_xsd_response)

    result = xmlschema.XMLSchema11('./templates/xsd/temp.xsd')
    return result
